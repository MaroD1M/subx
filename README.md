...
- **AI 翻译流水线**：纯 token 分块，串行处理，前文上下文传递，取消即时中止
- **智能校验**：全量接受非空译文，同源内容（音符/专有名词/繁简同形）自动标记进入核对
- **字幕核对中心**：支持逐条筛选、批量重译、手动编辑、调序，按状态/原因过滤
- **双语导出灵活切换**：核对页可覆盖输出模式(仅译文/双语/仅原文)、字幕样式、SRT/ASS 格式
- **实时预览**：SRT/ASS 内容随导出设置联动刷新
- **字幕来源支持完整**：支持视频内嵌字幕轨道提取，以及 `.srt`、`.ass`、`.ssa`、`.vtt` 等外挂字幕读取
- **任务日志持久化**：每个任务的完整执行日志保存到 `db/logs/` 目录，受日志保留天数控制
...
![SubX 首页工作区示例](docs/images/homepage-layout-example.png)
...
services:
  subx:
    image: ghcr.io/marod1m/subx:v1.2.0
    container_name: subx
...
> 📖 详细使用文档请参阅 [docs/usage.md](docs/usage.md) — 涵盖翻译设置、核对页操作、字幕格式、日志排查等完整指南。
