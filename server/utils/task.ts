...
export const TaskService = {
    async createTask(task: Partial<TranslationTask>): Promise<TranslationTask> {
      ...
      INSERT INTO tasks (
        task_id, file_path, source_type, track_index, model, target_lang, output_mode, style_preset, subtitle_format, subtitle_style_preset, bilingual_layout, status, progress, created_at, updated_at
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'), datetime('now'))
    `)
        stmt.run(
            task.taskId, task.filePath, task.sourceType, task.trackIndex,
            task.model, task.targetLanguage, task.outputMode, task.stylePreset || 'default',
    ...
    async process(taskId: string, openaiConfig: { apiKey: string, baseUrl?: string }) {
        ...
        const assOutputPath = join(videoDir, `${cleanName}.${task.targetLanguage}.ass`)
        ...
        try {
            ...
            const subtitleFormat = (task.subtitleFormat || 'srt') as 'srt' | 'ass' | 'both'
            const subtitleStylePreset = (task.subtitleStylePreset || 'bilingual_simple') as any
            const bilingualLayout = (task.bilingualLayout || 'translated_first') as any

            if (subtitleFormat === 'srt' || subtitleFormat === 'both') {
                await SubtitleService.writeSubtitle(
                    translatedEntries,
                    outputPath,
                    task.outputMode as 'translated' | 'bilingual',
                    subtitleStylePreset,
                    bilingualLayout
                )
            }
            if (subtitleFormat === 'ass' || subtitleFormat === 'both') {
                await SubtitleService.writeAssSubtitle(
                    translatedEntries,
                    assOutputPath,
                    task.outputMode as 'translated' | 'bilingual',
                    subtitleStylePreset,
            ...
            await this.updateStatus(taskId, 'exporting', 95, {
                log: subtitleFormat === 'both'
                    ? `文件保存成功: ${outputPath} + ${assOutputPath}`
                    : `文件保存成功: ${subtitleFormat === 'ass' ? assOutputPath : outputPath}`
            })
            ...
            db.prepare('UPDATE tasks SET status = \'done\', progress = 100, output_path = ?, updated_at = datetime(\'now\') WHERE task_id = ?')
                .run(subtitleFormat === 'ass' ? assOutputPath : outputPath, taskId)
