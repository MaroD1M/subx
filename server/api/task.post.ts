...
export default defineEventHandler(async (event) => {
    ...
    const {
        filePath, sourceType, trackIndex, targetLanguage, outputMode, model, stylePreset, files,
        subtitleFormat, subtitleStylePreset, bilingualLayout
    } = body
    ...
    const processFile = async (path: string, type: 'embedded' | 'external', track: number) => {
        ...
        await TaskService.createTask({
            ...
            outputMode: outputMode || config.outputMode || 'translated',
