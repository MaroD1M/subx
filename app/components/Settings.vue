<template>
  <div class="space-y-6">
    ...
    <div v-if="isInsecure" class="p-4 rounded-2xl bg-amber-50 dark:bg-amber-500/5 border border-amber-100 dark:border-amber-500/20 flex items-start gap-3">
      <UIcon name="i-lucide-shield-alert" class="w-5 h-5 text-amber-500 shrink-0 mt-0.5" />
      <div class="space-y-1">
        <p class="text-xs font-bold text-amber-700 dark:text-amber-400">当前连接未加密</p>
        <p class="text-[10px] text-amber-600 dark:text-amber-500/80 leading-relaxed">检测到正在通过 HTTP 访问。建议启用 HTTPS，以降低 API 密钥在传输过程中的风险。</p>
      </div>
    </div>
    ...
    <UFormField label="API 基础 URL" description="自定义 API 终点（例如 Ollama, One-API）。" class="w-full" :ui="formFieldUi">
    ...
    <!-- 默认模型独占一行 -->
    <UFormField label="默认模型" :ui="formFieldUi">
      <div class="space-y-2">
    ...
    <div class="grid grid-cols-2 gap-4">
      <UFormField label="目标语言" :ui="formFieldUi">
        <USelect v-model="config.targetLanguage" :items="['zh-CN', 'zh-TW', 'en', 'ja', 'ko']" class="w-full" />
      </UFormField>
      <UFormField label="输出模式" :ui="formFieldUi">
        <USelect v-model="config.outputMode" :items="[{ label: '仅显示译文', value: 'translated' }, { label: '双语对照', value: 'bilingual' }]" class="w-full" />
      </UFormField>
    </div>

    <div class="grid grid-cols-3 gap-4">
      <UFormField label="字幕格式" :ui="formFieldUi">
        <USelect
          v-model="config.subtitleFormat"
          :items="[
            { label: 'SRT', value: 'srt' },
            { label: 'ASS', value: 'ass' },
      ...
      <UFormField label="字幕样式预设" :ui="formFieldUi">
        <USelect
          v-model="config.subtitleStylePreset"
          :items="[
      ...
      <UFormField label="双语布局" :ui="formFieldUi">
        <USelect
          v-model="config.bilingualLayout"
          :items="[
            { label: '译文在上', value: 'translated_first' },
            { label: '原文在上', value: 'original_first' }
          ]"
          class="w-full"
          :disabled="config.outputMode !== 'bilingual'"
        />
      </UFormField>
    </div>

    <div class="grid grid-cols-4 gap-4 items-stretch">
      <UFormField label="分块大小 (Token)" description="较小的值可防止 AI 输出被截断。" class="flex flex-col h-full" :ui="{ ...formFieldUi, container: 'mt-auto' }">
        <UInputNumber v-model="config.chunkSize" :min="100" :max="4000" :step="100" class="w-full" />
      </UFormField>
      <UFormField label="并发任务数" description="同时进行的翻译请求数量。" class="flex flex-col h-full" :ui="{ ...formFieldUi, container: 'mt-auto' }">
        <UInputNumber v-model="config.concurrency" :min="1" :max="10" class="w-full" />
      </UFormField>
      <UFormField label="最大重试次数" description="翻译失败或漏译时自动重跑次数。" class="flex flex-col h-full" :ui="{ ...formFieldUi, container: 'mt-auto' }">
        <UInputNumber v-model="config.maxRetries" :min="0" :max="5" class="w-full" />
      </UFormField>
      <UFormField label="日志保留天数" description="AI 日志在 ai-logs 中的保留天数。" class="flex flex-col h-full" :ui="{ ...formFieldUi, container: 'mt-auto' }">
...
if (config.value) {
  ...
  if (!config.value.outputMode) config.value.outputMode = 'translated'
  if (!config.value.subtitleFormat) config.value.subtitleFormat = 'srt'
  if (!config.value.subtitleStylePreset) config.value.subtitleStylePreset = 'bilingual_simple'
  if (!config.value.bilingualLayout) config.value.bilingualLayout = 'translated_first'
}
...
async function save() {
  ...
  try {
    ...
    toast.add({ title: toastText.success, description: '设置已保存', color: 'success' })
    emit('close')
  } catch (e) {
    toast.add({
...
function applyRecommendedValues() {
  config.value.chunkSize = 5000
  config.value.concurrency = 3
  config.value.maxRetries = 3
  config.value.logRetentionDays = 15
  toast.add({ title: toastText.success, description: '已恢复推荐值', color: 'success' })
}
</script>
